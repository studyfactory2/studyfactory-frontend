...
import type { ManagerMemberLeavesState } from '../hooks/useManagerMemberLeaves';
import {
  MANAGER_LEAVE_REASONS,
  MANAGER_LEAVE_SLOTS,
  formatLeaveSlots,
  type ManagerSpecialLeaveSlot,
} from '../../leaves/leave-management-model';
import { ManagerLeaveCalendar } from './ManagerLeaveCalendar';
import '../styles/manager-member-leave.css';

type ManagerMemberLeavePanelProps = {
  initialDateKey?: string | null;
  initialMemberId?: number | null;
  initialSlot?: number | null;
  leaves: ManagerMemberLeavesState;
};

export function ManagerMemberLeavePanel({
  initialDateKey = null,
  initialMemberId = null,
  initialSlot = null,
  leaves,
}: ManagerMemberLeavePanelProps) {
  const [memberQuery, setMemberQuery] = useState('');
  const [selectedDates, setSelectedDates] = useState<ReadonlySet<string>>(
    () => new Set(initialDateKey ? [initialDateKey] : []),
  );
  const [selectedSlots, setSelectedSlots] = useState<ReadonlySet<number>>(
    () => new Set(initialSlot ? [initialSlot] : []),
  );
  const [reason, setReason] = useState('');
  const [customReason, setCustomReason] = useState('');
  const [formError, setFormError] = useState<string | null>(null);
  const [deleteTarget, setDeleteTarget] =
    useState<ManagerSpecialLeaveSlot | null>(null);
  ...
  const filteredMembers = useMemo(() => {
    const query = memberQuery.trim().toLocaleLowerCase('ko');

    if (!query) {
      return leaves.member.items;
    }

    return leaves.member.items.filter((member) => {
      const seatLabel =
        member.seatNumber === null ? roleLabel(member.role) : member.seatNumber;

      return (
        member.name.toLocaleLowerCase('ko').includes(query) ||
        String(seatLabel).toLocaleLowerCase('ko').includes(query) ||
        roleLabel(member.role).includes(query)
      );
    });
  }, [leaves.member.items, memberQuery]);
  const selectedMemberInResults =
    selectedMember !== null &&
    filteredMembers.some((member) => member.id === selectedMember.id);
  const awaitingSearchSelection =
    memberQuery.trim().length > 0 && !selectedMemberInResults;
  ...
  if (leaves.member.items.length === 0) {
    return (
  ...
  return (
    <div className="manager-member-leave">
      <div className="manager-member-leave__member-row">
        <div className="manager-member-leave__picker">
          <label className="manager-member-leave__search">
            <span className="manager-member-leave__search-label">
              사원 검색
            </span>
            <span className="manager-member-leave__search-control">
          ...
          >
            {(id) => (
              <Select
                ...
                value={selectedMemberInResults ? selectedMember.id : ''}
              >
                <option disabled value="">
                  {filteredMembers.length === 0
                    ? '검색 결과 없음'
                    : `${filteredMembers.length}명 중 선택`}
                </option>
        ...
        <div className="manager-member-leave__selection-summary">
          <span>선택</span>
      ...
      {leaves.member.unavailable ||
      selectedMember === null ||
      awaitingSearchSelection ? (
        <div className="manager-member-leave__unavailable">
          <SectionEmpty
            title={
              awaitingSearchSelection
                ? '검색 결과에서 사원을 선택해 주세요'
          ...
          >
            <p>
              {awaitingSearchSelection
                ? '위 목록에서 관리할 사원을 선택하면 휴무 달력이 열려요.'
                : '현재 운영 지점의 등록 사원인지 확인한 뒤 다시 선택해 주세요.'}
            </p>
          </SectionEmpty>
        </div>
      ) : (
        <>
          {initialMemberId === selectedMember.id &&
            initialDateKey &&
            initialSlot && (
              <p className="manager-member-leave__source-context">
                출석부에서 {formatKoreanDate(initialDateKey)} {initialSlot}
                교시를 불러왔어요. 등록 전 날짜와 사유를 확인해 주세요.
              </p>
            )}

          <div className="manager-member-leave__workspace">
            <div className="manager-member-leave__calendar-column">
            ...
            <section className="manager-member-leave__editor">
              <header>
              ...
              <fieldset className="manager-member-leave__slots">
                <legend>교시</legend>
                <div>
                ...
                <button
                  className="manager-member-leave__all-slots"
                  onClick={() => {
                    setSelectedSlots(
              ...
              <div className="manager-member-leave__reason">
              ...
              {reason === '기타' && (
                <div className="manager-member-leave__custom-reason">
                  <Field
              ...
              {formError && (
                <p className="manager-member-leave__form-error" role="alert">
                  {formError}
                </p>
              )}
              ...
              <Button
                full
                className="manager-member-leave__submit"
          ...
          <section className="manager-special-leave-list">
            <header>
              <div>
                <h4>특별 휴무 내역</h4>
                ...
                {leaves.entries.items.map((item) => (
                  <li key={item.key}>
                    ...
                    <span>
                      ...
                      {item.recurring && (
                        <Badge tone="neutral">반복 기록</Badge>
                      )}
                    </span>
                    {item.recurring ? (
                      <span className="manager-special-leave-list__managed">
                        고정·반복 관리
                      </span>
                    ) : (
                      <button
